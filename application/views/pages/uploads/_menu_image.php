...
<div class="panel panel-primary">
	...
	<?php if ($images): ?>
		<?php foreach ($images as $image): ?>
			...
			<a href="<?php echo(base_url('uploads/menu/'.$image->image_name)); ?>" class="image-link">
				<img src="<?php echo(base_url('uploads/menu/'.$image->image_name)); ?>" class="img-responsive img-thumbnail" alt="Cinque Terre" style="width:204px;height:auto;">
			</a>
		</div><?php endforeach ?>
	<?php else: ?>
...
<script type="text/javascript">
	$(document).ready(function() {
	    ...
	    $('body').on('click','.delete',function() {
			...
			var url='<?php echo(site_url()) ?>/menu/delete/'+image_id+'/'+'<?php echo $this->uri->segment(3) ?>';
